...
typedef std::map <int, fileinfo_t> extmapvar_t;
...
public:
    ...
    void dumpdebugmap(std::ostream & out);
private:
    ...
    typedef pasmo_impl::extmapvar_t extmapvar_t;
    extmapvar_t extmapvar;
...
void Asm::In::gendata (byte data)
{
	...
	if (current > maxused)
		maxused= current;
	mem [current]= data;
    // store the address of the current line and the filename
    // so the debugger can select the correct line in the code editor 
    fileinfo_t fi = getfileinfo(getline());
    extmapvar[current].line = fi.line;
    extmapvar[current].file = fi.file;
	++current;
}
...
void Asm::In::emitsna(std::ostream & out)
{
    ...
    out.put(0); // IM
    out.put(0); // Border Color

    // write clear memory from start of ram to minused
    for (int i=16384; i < minused; ++i)
    {
        out.put(0);
    }
...
std::string& getfilename(std::string& file)
{
    ...
    {
        file.erase(0, last_slash + 1);
    }
    return file;
}

void Asm::In::dumpdebugmap(std::ostream & out)
{
	for(extmapvar_t::iterator it = extmapvar.begin(); it != extmapvar.end(); ++it)
	{
		out << hex4(it->first) << "\t" << it->second.line << "\t" << getfilename(it->second.file) << endl;
	}
}
...
void Asm::dumpdebugmap(std::ostream & out)
{
	pin->dumpdebugmap(out);
}
// End of asm.cpp
