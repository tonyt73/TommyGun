...
const string pasmoversion = "1.0";//(VERSION);
...
public:
    ...
    bool debug() const { return emitdebug; }
...
int doit (int argc, char * * argv)
{
    ...
    assembler.emitsna(out);
#else
	// Fails to compile in Borland C++ Compiler
    ...
    // output the debug if the map file is present
	if(! option.publiconly() && ! filesymbol.empty() )
	{
		std::ofstream sout;
		std::streambuf * cout_buf= 0;
        // change map file extension to .dbg file extension
        std::string filemapext = filesymbol.substr(0, filesymbol.find(".")) + ".dbg";
		if(filesymbol != "-")
		{
			sout.open(filemapext.c_str() );
			if(! sout.is_open() )
				throw SymbolFileError;
			cout_buf= cout.rdbuf();
			cout.rdbuf(sout.rdbuf() );
		}
		assembler.dumpdebugmap(cout);
		if (cout_buf)
		{
			cout.rdbuf (cout_buf);
			sout.close ();
...
#if defined(CPPBUILDER)
std::string Pasmo::ChangeExt(const std::string& infile, const std::string& ext)
{
    return infile.substr(0, infile.find(".") + 1) + ext;
}

bool Pasmo::Compile(const std::string& infile, std::string& output)
{
    // build arguments
    const int argc = 7;
    std::string snaFile = ChangeExt(infile, "sna");
    std::string mapFile = ChangeExt(infile, "map");
    std::string args[argc] =
    {
        "pasmo.exe", "-v", "--debug", "--sna", infile, snaFile, mapFile
    };
    ...
    return compile(argc, (char**)&argv) == 0;
}
#endif
// End of pasmo.cpp
