...
public:
    ...
    fileinfo_t AsmFile::In::getfileinfo(size_t nline) const;
...
fileinfo_t AsmFile::In::getfileinfo(size_t nline) const
{
    ...
    fileinfo_t fi;
    fi.line = fileref.numline(linf.getfileline())+ 1;
    fi.file = fileref.name();
    return fi;
}
...
fileinfo_t AsmFile::getfileinfo(size_t nline) const
{
    return in().getfileinfo(nline);
}
