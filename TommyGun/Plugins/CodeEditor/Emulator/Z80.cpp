...
#define M_POP(Rg)      \
  R->Rg.B.l=RdZ80(R->SP.W++);R->Rg.B.h=RdZ80(R->SP.W++);StackOp(R, soPop, "", 0, 0, 0)
//#define M_POP(Rg)      \
//  R->Rg.B.l=RdZ80(R->SP.W++);R->Rg.B.h=RdZ80(R->SP.W++)
#define M_PUSH(Rg)     \
  WrZ80(--R->SP.W,R->Rg.B.h);WrZ80(--R->SP.W,R->Rg.B.l);StackOp(R, soPush, QUOTEME(Rg), R->Rg.W, R->SP.W, 0)
//#define M_PUSH(Rg)     \
...
#define M_CALL         \
  ...
  StackOp(R, soCall, "call", R->PC.W, R->SP.W, J.W);        \
  R->PC.W=J.W
//#define M_CALL         \
//  J.B.l=RdZ80(R->PC.W++);J.B.h=RdZ80(R->PC.W++);         \
//  WrZ80(--R->SP.W,R->PC.B.h);WrZ80(--R->SP.W,R->PC.B.l); \
...
#define M_RET R->PC.B.l=RdZ80(R->SP.W++);R->PC.B.h=RdZ80(R->SP.W++);StackOp(R, soRet, "", 0, 0, 0)
...
void IntZ80(Z80 *R,word Vector)
{
  if((R->IFF&IFF_1)||(Vector==INT_NMI))
  {
    ...
    switch(Vector)
    {
      ...
      case INT_RST00: DebugString(R);break;
