...
//-- PLUGIN -----------------------------------------------------------------
ZXImageParserPlugin* g_Plugin = NULL;
// Plugin Class Factory function
extern ZXImageParserPlugin* NewPluginClass(void);
...
//---------------------------------------------------------------------------
FUNCTION_PREFIX ParseResources(KXmlInfo*& XmlData, String& Resource, TStringList*& Code)
{
    RL_METHOD
    RL_HRESULT(E_FAIL)
    if (true == SAFE_PTR(g_Plugin))
    {
        hResult = g_Plugin->ParseResources(XmlData, Resource, Code);
    }
    return hResult;
}
//---------------------------------------------------------------------------
...
{
    sDescription = "Images (Sprites, Tiles and Screen$)";
    return S_OK;
}
//---------------------------------------------------------------------------
