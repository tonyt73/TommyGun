...
//-- CONSTANTS --------------------------------------------------------------
const String g_sPluginTitle = "Tables";
//---------------------------------------------------------------------------
// Initialise
/**
 ...
 */
//---------------------------------------------------------------------------
HRESULT __fastcall TfrmMSXTablesEditor::Initialise(TZX_HPLUGIN PluginHandle)
{
    RL_METHOD
    RL_HRESULT(S_OK)
 ...
 */
//---------------------------------------------------------------------------
HRESULT __fastcall TfrmMSXTablesEditor::Release(void)
{
    RL_METHOD
    RL_HRESULT(S_OK)
...
//---------------------------------------------------------------------------
void __fastcall TfrmMSXTablesEditor::AttachToolbars(void)
{
    /*for (int i = 0; i < Application->MainForm->ControlCount; ++i)
    {
        ...
        if (true == SAFE_PTR(ControlBar))
        {
            ...
            tbrMSXTablesTools->Visible    = true;//S_OK == IsActivePlugin(m_PluginHandle);
            tbrMSXTablesTools->Parent     = ControlBar;
            tbrMSXTablesTools->Left       = 200;
        }
    }*/
    ...
    if (ViewMenu)
    {
        ...
        if (Toolbars)
        {
            m_MSXTablesTools              = new TMenuItem(NULL);
            m_MSXTablesTools->Caption     = "MSXTables Tools";
            m_MSXTablesTools->Checked     = true;
            m_MSXTablesTools->Tag         = (int)tbrMSXTablesTools;
            m_MSXTablesTools->OnClick     = OnMenuClick;
            m_MSXTablesTools->Visible     = true;//S_OK == IsActivePlugin(m_PluginHandle);
            Toolbars->Add(m_MSXTablesTools);
        }
    }*/
}
//---------------------------------------------------------------------------
void __fastcall TfrmMSXTablesEditor::OnMenuClick(TObject* Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TfrmMSXTablesEditor::TrapTabEvents(TWinControl* Control)
{
...
//---------------------------------------------------------------------------
void __fastcall TfrmMSXTablesEditor::OnTabHide(TObject *Sender)
{
    //tbrMSXTablesTools->Visible = false;
    //m_MSXTablesTools->Visible  = false;
}
//---------------------------------------------------------------------------
void __fastcall TfrmMSXTablesEditor::OnTabShow(TObject *Sender)
{
    //m_MSXTablesTools->Visible  = true;
    //tbrMSXTablesTools->Visible = m_MSXTablesTools->Checked;
}
//---------------------------------------------------------------------------
