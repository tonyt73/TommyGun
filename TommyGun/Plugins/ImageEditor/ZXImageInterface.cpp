...
void __fastcall ZXImageInterface::GetInterfaces(HINSTANCE hInstance)
{
    ...
    m_NotifyFunc                = reinterpret_cast<IENotifyPtr              >(GetProcAddress(hInstance, "IENotify"   ));
    ...
    m_GetImageFunc              = reinterpret_cast<ImageGetCurrentPtr       >(GetProcAddress(hInstance, "ImageGetCurrent"           ));
    if(m_GetImageFunc)              ZX_LOG_INFO(lfPlugin, "ImageGetCurrent() found") else ZX_LOG_ERROR(lfPlugin, "ImageGetCurrent() NOT found");
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
HRESULT __fastcall ZXImageInterface::ImageGetCurrent(TZX_HPLUGIN PluginHandle, ZXImage*& pImage)
{
    RL_HRESULT(E_POINTER);
    if (FALSE == IsBadCodePtr(reinterpret_cast<StdCallPtr>(m_GetImageFunc)))
    {
        hResult = m_GetImageFunc(PluginHandle, pImage);
    }
    return hResult;
}
//---------------------------------------------------------------------------
