...
//---------------------------------------------------------------------------
const   String  g_sPaletteName = "Sam Coupe Mode 1";
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
__fastcall SamMode1Palette::SamMode1Palette()
...
//---------------------------------------------------------------------------
void __fastcall SamMode1Palette::Initialize(void)
{
...
//---------------------------------------------------------------------------
void __fastcall SamMode1Palette::SetInk(unsigned char iInk)
{
    m_iInk       = iInk;
    m_ColorPen   = g_ColorsSpectrum[m_iInk + (8 * m_iBright)];
}
//---------------------------------------------------------------------------
void __fastcall SamMode1Palette::SetPaper(unsigned char iPaper)
{
    m_iPaper     = iPaper;
    m_ColorBrush = g_ColorsSpectrum[m_iPaper + (8 * m_iBright)];
}
//---------------------------------------------------------------------------
void __fastcall SamMode1Palette::SetFlash(unsigned char iFlash)
{
    m_iFlash     = iFlash;
}
//---------------------------------------------------------------------------
void __fastcall SamMode1Palette::SetBright(unsigned char iBright)
{
    m_iBright    = iBright;
    m_ColorPen   = g_ColorsSpectrum[m_iInk   + (8 * m_iBright)];
    m_ColorBrush = g_ColorsSpectrum[m_iPaper + (8 * m_iBright)];
...
//---------------------------------------------------------------------------
void __fastcall SamMode1Palette::SetColorAt(ZXImage& Image, int X, int Y)
{
...
//---------------------------------------------------------------------------
void __fastcall SamMode1Palette::GetColorAt(ZXImage& Image, int X, int Y)
{
...
//---------------------------------------------------------------------------
void __fastcall SamMode1Palette::Invert(ZXImage& Image, int X, int Y)
{
    if (0 <= X && X < Image.Width && 0 <= Y && Y < Image.Height)
    {
        // swap the attributes
...
//---------------------------------------------------------------------------
void __fastcall SamMode1Palette::Swap(ZXImage& Image, int X, int Y)
{
    if (0 <= X && X < Image.Width && 0 <= Y && Y < Image.Height)
    {
...
//---------------------------------------------------------------------------
void __fastcall SamMode1Palette::Write(int X, int Y, bool bSet, ZXImage& Image)
{
    // dirty the character block
    int dX = (X >> 3) << 3;
    int dY = (Y >> 3) << 3;
...
//---------------------------------------------------------------------------
void __fastcall SamMode1Palette::Draw(ZXImage& Image, Graphics::TBitmap* Bitmap)
{
    //PERFORMANCE_PUSH
...
//---------------------------------------------------------------------------
void __fastcall SamMode1Palette::SetPixel(int iX, int iY, unsigned char iInk, unsigned char iPaper, unsigned char iBright, unsigned char iFlash, bool bSet, ZXImage& Image)
{
    //PERFORMANCE_PUSH
...
//---------------------------------------------------------------------------
void __fastcall SamMode1Palette::SetPixel(int iX, int iY, unsigned char PixelByte, ZXImage* Image)
{
    if (0 <= iX && iX < Image->Width && 0 <= iY && iY < Image->Height)
    {
...
//---------------------------------------------------------------------------
void __fastcall SamMode1Palette::SetAttribute(int iX, int iY, unsigned char iInk, unsigned char iPaper, unsigned char iBright, unsigned char iFlash, ZXImage& Image)
{
    if (0 <= iX && iX < Image.Width && 0 <= iY && iY < Image.Height)
    {
        int iCols = CalculateStride(Image.Width);
...
//---------------------------------------------------------------------------
unsigned char __fastcall SamMode1Palette::GetAttribute(int iX, int iY, ZXImage& Image)
{
...
//---------------------------------------------------------------------------
void __fastcall SamMode1Palette::SetAttribute(int iX, int iY, unsigned char Attribute, ZXImage* Image)
{
    if (0 <= iX && iX < Image->Width && 0 <= iY && iY < Image->Height)
    {
        int iCharWidth = CalculateStride(Image->Width);
        Image->Attributes[((iY / 8) * iCharWidth) + (iX / 8)] = Attribute;
    }
}
//---------------------------------------------------------------------------
bool __fastcall SamMode1Palette::Convert(ZXImage* Image, Graphics::TBitmap* Bitmap)
{
...
//---------------------------------------------------------------------------
void __fastcall SamMode1Palette::FindColors(Graphics::TBitmap* Bitmap, int iX, int iY, unsigned char& Ink, unsigned char& Paper)
{
...
//---------------------------------------------------------------------------
void __fastcall SamMode1Palette::SetColors(ZXImage* Image, Graphics::TBitmap* Bitmap, int iX, int iY, unsigned char Ink, unsigned char Paper)
{
...
//---------------------------------------------------------------------------
unsigned char __fastcall SamMode1Palette::GetColorIndex(TColor Color)
{
    unsigned char ColorIndex = 0;
    for (int i = 0; i < 16; i++)
    {
        if (Color == g_ColorsSpectrum[i])
        {
            ColorIndex = i;
            break;
        }
...
//---------------------------------------------------------------------------
String __fastcall SamMode1Palette::GetName(void)
{
    return g_sPaletteName;
}
//---------------------------------------------------------------------------
String __fastcall SamMode1Palette::GetColorAsString(ZXImage& Image, int X, int Y)
{
    String sColor("None");
    if (0 <= X && X < Image.Width && 0 <= Y && Y < Image.Height)
    {
