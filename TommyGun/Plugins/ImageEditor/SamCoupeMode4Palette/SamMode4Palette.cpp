...
//---------------------------------------------------------------------------
__fastcall Mode4Palette::Mode4Palette()
: ZXPalette(16, 256, 192, true),
  m_iColor(1)
{
...
//---------------------------------------------------------------------------
void __fastcall Mode4Palette::Initialize(void)
{
...
//---------------------------------------------------------------------------
void __fastcall Mode4Palette::SetColor(unsigned char iColor)
{
    if (iColor < 16)
    {
        m_iColor = iColor;
...
//---------------------------------------------------------------------------
void __fastcall Mode4Palette::SetTableColor(unsigned char iColor, TColor Color)
{
    if (iColor < 16)
    {
...
//---------------------------------------------------------------------------
unsigned char __fastcall Mode4Palette::GetColor(void)
{
    return m_iColor;
}
//---------------------------------------------------------------------------
TColor __fastcall Mode4Palette::GetColor(unsigned char iColor)
{
    return ZXPalette::GetColor(iColor);
}
//---------------------------------------------------------------------------
void __fastcall Mode4Palette::SetColorAt(ZXImage& Image, int X, int Y)
{
    Write(X, Y, true, Image);
}
//---------------------------------------------------------------------------
void __fastcall Mode4Palette::GetColorAt(ZXImage& Image, int X, int Y)
{
...
//---------------------------------------------------------------------------
void __fastcall Mode4Palette::Write(int X, int Y, bool bSet, ZXImage& Image)
{
...
//---------------------------------------------------------------------------
void __fastcall Mode4Palette::Draw(ZXImage& Image, Graphics::TBitmap* Bitmap)
{
    //PERFORMANCE_PUSH
    ...
    for (int rect = 0; rect < iRects; ++rect)
    {
        ...
        for (int y = UpdateRect.Top; y < UpdateRect.Bottom; ++y)
        {
            for (int x = UpdateRect.Left; x < UpdateRect.Right; x += m_fPixelsPerByte)
            {
                ...
                Bitmap->Canvas->Pixels[x+1][y] = GetColor((Pixels >> 0) & 0x0F);
            }
        }
    }
...
//---------------------------------------------------------------------------
void __fastcall Mode4Palette::SetPixel(int iX, int iY, unsigned char iColor, bool bSet, ZXImage& Image)
{
    //PERFORMANCE_PUSH
...
//---------------------------------------------------------------------------
String __fastcall Mode4Palette::GetColorAsString(ZXImage& Image, int X, int Y)
{
    String sColor("None");
    if (0 <= X && X < Image.Width && 0 <= Y && Y < Image.Height)
    {
...
//---------------------------------------------------------------------------
void __fastcall Mode4Palette::Reset(void)
{
...
//---------------------------------------------------------------------------
void __fastcall Mode4Palette::Loaded(void)
{
    frmSamMode4Palette->RefreshPanels();
}
//---------------------------------------------------------------------------
