...
//---------------------------------------------------------------------------
TfrmSamMode4Palette *frmSamMode4Palette = NULL;
//---------------------------------------------------------------------------
__fastcall TfrmSamMode4Palette::TfrmSamMode4Palette(TComponent* Owner)
    : TForm(Owner)
{
}
//---------------------------------------------------------------------------
HRESULT __fastcall TfrmSamMode4Palette::Initialize(TZX_HPLUGIN PluginHandle, HINSTANCE hParentInstance)
{
    RL_HRESULT(S_OK);
...
//---------------------------------------------------------------------------
HRESULT __fastcall TfrmSamMode4Palette::Release(void)
{
    RL_HRESULT(S_OK);
    panSamPalette->Parent->RemoveControl(panSamPalette);
    return hResult;
}
//---------------------------------------------------------------------------
HRESULT __fastcall TfrmSamMode4Palette::GetPalette(ZXPalette*& Palette)
{
    RL_METHOD
    RL_HRESULT(S_OK);
    Palette = NULL;
...
//---------------------------------------------------------------------------
void __fastcall TfrmSamMode4Palette::panColour0Click(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TfrmSamMode4Palette::lblColours0to7Click(TObject *Sender)
{
    TLabel* Label = dynamic_cast<TLabel*>(Sender);
    if (true == SAFE_PTR(Label) && true == dlgSamCoupeColors->Execute(m_Palette.Color))
    {
        m_Palette.SetTableColor(m_Palette.Color, dlgSamCoupeColors->Color);
        for (int i = 0; i < panSamPalette->ControlCount; i++)
        {
            TPanel* Panel = dynamic_cast<TPanel*>(panSamPalette->Controls[i]);
...
//---------------------------------------------------------------------------
void __fastcall TfrmSamMode4Palette::OnUpdatePaletteGUI(void)
{
...
//---------------------------------------------------------------------------
void __fastcall TfrmSamMode4Palette::lblResetPaletteClick(TObject *Sender)
{
    m_Palette.Reset();
...
//---------------------------------------------------------------------------
void __fastcall TfrmSamMode4Palette::RefreshPanels(void)
{
