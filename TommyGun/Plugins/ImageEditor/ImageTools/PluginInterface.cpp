...
#pragma hdrstop
//-- PLUGIN -----------------------------------------------------------------
ZXImageTools* g_Plugin = NULL;
// Plugin Class Factory function
extern ZXImageTools* NewPluginClass(void);
...
//---------------------------------------------------------------------------
FUNCTION_PREFIX MouseDown(ZXImage* Image, TMouseButton Button, TShiftState Shift, int X, int Y)
{
    RL_METHOD
    RL_HRESULT(E_FAIL)
    hResult = g_Plugin->MouseDown(Image, Button, Shift, X, Y);
    return hResult;
}
//---------------------------------------------------------------------------
FUNCTION_PREFIX MouseMove(ZXImage* Image, TShiftState Shift, int X, int Y)
{
    //RL_METHOD
    RL_HRESULT(E_FAIL)
    hResult = g_Plugin->MouseMove(Image, Shift, X, Y);
    return hResult;
}
//---------------------------------------------------------------------------
FUNCTION_PREFIX MouseUp(ZXImage* Image, TMouseButton Button, TShiftState Shift, int X, int Y)
{
    RL_METHOD
    RL_HRESULT(E_FAIL)
    hResult = g_Plugin->MouseUp(Image, Button, Shift, X, Y);
    return hResult;
}
//---------------------------------------------------------------------------
FUNCTION_PREFIX Cursor(TCursor& Cursor)
{
    RL_METHOD
    RL_HRESULT(E_FAIL)
    hResult = g_Plugin->Cursor(Cursor);
    return hResult;
}
//---------------------------------------------------------------------------
FUNCTION_PREFIX ImageChange(ZXImage* Image)
{
    RL_METHOD
    RL_HRESULT(E_FAIL)
    hResult = g_Plugin->ImageChange(Image);
    return hResult;
}
//---------------------------------------------------------------------------
