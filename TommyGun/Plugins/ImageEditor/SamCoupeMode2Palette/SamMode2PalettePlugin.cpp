...
{
    RL_METHOD
    frmSamMode2Palette = new TfrmSamMode2Palette(NULL);
}
//---------------------------------------------------------------------------
// Destructor
/**
...
__fastcall ZXSpectrumFormat::~ZXSpectrumFormat()
{
    ...
    SAFE_DELETE(frmSamMode2Palette);
    Release();
}
//---------------------------------------------------------------------------
// InitialisePlugin
...
HRESULT __fastcall ZXSpectrumFormat::InitialisePlugin(ZXPlugin* pPlugin)
{
    ...
    if (true == SAFE_PTR(frmSamMode2Palette))
    {
        hResult = frmSamMode2Palette->Initialize(pPlugin->Handle, pPlugin->ParentInstance);
    }
    return hResult;
}
//---------------------------------------------------------------------------
...
HRESULT __fastcall ZXSpectrumFormat::ReleasePlugin(void)
{
    ...
    if (true == SAFE_PTR(frmSamMode2Palette))
    {
        hResult = frmSamMode2Palette->Release();
    }
    return hResult;
}
//---------------------------------------------------------------------------
...
HRESULT __fastcall ZXSpectrumFormat::GetPalette(ZXPalette*& Palette)
{
    ...
    if (true == SAFE_PTR(frmSamMode2Palette))
    {
        hResult = frmSamMode2Palette->GetPalette(Palette);
    }
    return hResult;
}
//---------------------------------------------------------------------------
HRESULT __fastcall ZXSpectrumFormat::GetPalettePanel(TPanel*& Panel)
{
    ...
    if (true == SAFE_PTR(frmSamMode2Palette))
    {
        Panel = frmSamMode2Palette->panSamMode2Palette;
        hResult = S_OK;
    }
    return hResult;
}
