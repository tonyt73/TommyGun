...
//-- CONSTANTS --------------------------------------------------------------
const String g_sTabTitle = "Music";
//---------------------------------------------------------------------------
// Initialise
/**
 ...
 */
//---------------------------------------------------------------------------
HRESULT __fastcall TfrmMusicEditor::Initialise(TZX_HPLUGIN PluginHandle)
{
    RL_METHOD
    RL_HRESULT(S_OK)
    ...
    hResult = S_FALSE;
    if (S_OK == AddPage(PluginHandle, tmpWindow, g_sTabTitle))
    {
        if (NULL != tmpWindow->Handle)
        {
            ...
            m_bInitialized = true;
            panMusicContainer->DoubleBuffered = true;
            panMusicScroller->DoubleBuffered = true;
            panKeyboardScroller->DoubleBuffered = true;
            panPianoToolbarContainer->DoubleBuffered = true;
            panVolumeScroller->DoubleBuffered = true;
            m_Keyboard.Init();
            m_Timeline.Init();
            m_Music.Init();
            m_Volume.Init();
 ...
 */
//---------------------------------------------------------------------------
HRESULT __fastcall TfrmMusicEditor::Release(void)
{
    RL_METHOD
    RL_HRESULT(S_OK)
