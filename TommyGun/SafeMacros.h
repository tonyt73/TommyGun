...
//---------------------------------------------------------------------------
extern std::vector<TZXExceptionInfo>   g_Exceptions;
//---------------------------------------------------------------------------
void __fastcall PushException(const String sMessage, const String sFile, const String sFunc, int iLine);
//---------------------------------------------------------------------------
#if defined(USE_PROTECTION) && !defined(PROTECT_BEGIN)
#define PROTECT_BEGIN __try { try {
#else
#define PROTECT_BEGIN
#endif//PROTECT_BEGIN
//---------------------------------------------------------------------------
#if defined(USE_PROTECTION) && !defined(PROTECT_END)
#define PROTECT_END } catch(Exception& E)\
{\
    PushException(E.Message, __FILE__, __FUNC__, __LINE__);\
}\
catch(...)\
{\
    PushException("Unknown C++ Exception ", __FILE__, __FUNC__, __LINE__);\
} }\
__except(true)\
{\
    PushException("Unknown Structured Exception ", __FILE__, __FUNC__, __LINE__);\
}
#else
#define PROTECT_END
#endif//PROTECT_END
//---------------------------------------------------------------------------
#if defined(USE_PROTECTION) && !defined(PROTECT_PUSH)
#define PROTECT_PUSH(E) PushException(E.Message, __FILE__, __FUNC__, __LINE__);
#else
#define PROTECT_PUSH
#endif
//---------------------------------------------------------------------------
